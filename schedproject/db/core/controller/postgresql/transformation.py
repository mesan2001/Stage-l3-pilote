    ...
    def copy_staging_result_to_warehouse(
        ...
        for expression in config.datamarts:
            ...
            if not target_exists:
                ...
                # Table exists - add new columns and update/insert data
                ...
                # Merge data from temp table into the main table
                if primary_key_column:
                    # For tables with a primary key, we can do an upsert
                    columns_to_update = [
                    ...
                    # For tables without a primary key, direct insert
                    insert_query = f"""
                        INSERT INTO {to_view} ({', '.join(new_schema.keys())})
                        SELECT {', '.join(new_schema.keys())}
                        FROM {temp_table};
