...
from routes import FormationsRoutes
...
class FormationsServices(AbstractService):
    ...
    def get_formation_given_step(self, step_id: ID) -> dict[str, Any]:
        try:
            step_result = self.app.steps_service.get_by_id(step_id)
            ...
            """
            formation_result = self.app.db_service.execute_query(
                formation_query, [step_id]
            )

            if not formation_result:
                raise ValueError(f"No formation found containing step {step_id}")

            return dict(formation_result[0])

        except Exception as e:
            logging.error(f"Error retrieving formation for step {step_id}: {str(e)}")
            raise
