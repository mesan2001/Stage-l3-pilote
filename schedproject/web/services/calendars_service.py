...
from routes import CalendarsRoutes, PeriodsRoutes
...
class PeriodsService(AbstractService):
    ...
    def _validate(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for exclusion in exclusions:
            if isinstance(exclusion, list) and len(exclusion) == 2:
                try:
                    # Validate both dates in the exclusion range
                    start_date = (
                        date.fromisoformat(exclusion[0])
                        if isinstance(exclusion[0], str)
                        else None
                    )
                    end_date = (
                        date.fromisoformat(exclusion[1])
                        if isinstance(exclusion[1], str)
                        else None
                    )
                    ...
                    if start_date and end_date:
                        ...
                        raise ValueError(
                            f"Invalid date format in exclusion pair: {exclusion}"
                        )
                except ValueError:
                    raise ValueError(
                        f"Invalid date format in exclusion pair: {exclusion}"
                ...
                raise ValueError(
                    "Exclusions must contain date strings or pairs of date strings"
                )
